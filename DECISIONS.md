...
**The actual fix stops guessing and makes the failure legible.** Rather than a third blind hypothesis, `loadFarmSites()` moved inside the handler's `try`. Whatever throws — a missing import attribute, a file the deployment bundle doesn't actually contain, anything else — now lands in `body.errors` in a normal `200` response instead of taking the whole function down, which is both a genuine fix (the route stops hard-crashing, matching its own stated design of failing shut rather than failing loud) and the instrument that will show the real error text on the next deploy if something is still wrong.

**Verified:** typecheck clean. `/api/windspeed` returns `200`, `health: "ok"`, live per-farm speeds for all 76 farms in local dev. Not yet re-verified against production (would need a deploy) — flagged explicitly rather than claimed, after the first fix's "should work" turned out not to.
