 ...
 * carry no data and are not narration test cases), for each southern region
 * the product now narrates: South England (`/`) and England (`/map`,
 * Windfall_Map_Spec.md step 3b Part B). Without ANTHROPIC_API_KEY set this
 * prints the facts and the prompt for each fixture/region pair and stops
 * there, so the prompt can be read and edited without spending a call. With
 * a key, it calls the model and prints the validator's verdict alongside the
 * text — this transcript is the phase 2 prompt-iteration log.
 *
...
import { situationOf, type SouthernRegion } from '../src/lib/situation';
...
const REGIONS: SouthernRegion[] = ['south-england', 'england'];
